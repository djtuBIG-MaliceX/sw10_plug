...
def  main():
  ...
  installer = "\installer\SW10_PLUG Installer.exe"
   
  if demo:
    installer = "\installer\SW10_PLUG Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SW10_PLUG manual.pdf" 
  ]
  ...
  ZIPNAME = "SW10_PLUG-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SW10_PLUG-v" + FULLVERSIONSTR + "-win-demo.zip"
